...
func ArraySort(array interface{}, field string, reverse ...bool) {
	...
	for index := 0; index < srcValue.Elem().Len(); index++ {
		...
		var numFiled int
		var err error
		if sortFiled.Kind() == reflect.Float64 {
			numFiled = int(sortFiled.Float())
		} else {
			numFiled, err = strconv.Atoi(fmt.Sprint(sortFiled.Interface()))
			if err != nil {
				panic("ArraySort field获取int失败" + field + ": " + err.Error())
			}
		}
...
// IsItemInSlice 是否在
func IsItemInSlice(idstr string, idargs int) (exist bool) {
	tmpids := strings.Split(idstr, ",")
	for index := 0; index < len(tmpids); index++ {
		tmpid, _ := strconv.Atoi(tmpids[index])
		if idargs == tmpid {
			exist = true
			break
		}
...
// 结构体赋值
func StructConver(a interface{}, b interface{}) (err error) {
