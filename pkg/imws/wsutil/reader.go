...
type FrameHandlerFunc func(imws.Header, io.Reader) error
...
type Reader struct {
	...
	opCode imws.OpCode      // Used to store message op code on fragmentation.
...
func NewReader(r io.Reader, s imws.State) *Reader {
	return &Reader{
		Source: r,
		State:  s,
	}
...
func (r *Reader) NextFrame() (hdr imws.Header, err error) {
	hdr, err = imws.ReadHeader(r.Source)
	...
	if err == nil && !r.SkipHeaderCheck {
		err = imws.CheckHeader(hdr, r.State)
	}
	if err != nil {
		return hdr, err
	}
	...
	if r.CheckUTF8 && (hdr.OpCode == imws.OpText || (r.fragmented() && r.opCode == imws.OpText)) {
		r.utf8.Source = frame
		frame = &r.utf8
	}
	...
	if hdr.OpCode == imws.OpContinuation {
	...
	if hdr.Fin {
		r.State = r.State.Clear(imws.StateFragmented)
	} else {
		r.State = r.State.Set(imws.StateFragmented)
	}
...
func NextReader(r io.Reader, s imws.State) (imws.Header, io.Reader, error) {
	rd := &Reader{
		Source: r,
		State:  s,
	}
