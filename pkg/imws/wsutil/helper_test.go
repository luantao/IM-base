	...
	for _, test := range []struct {
		...
		},
		{
			source: func() io.Reader {
				...
				var buf bytes.Buffer
				fs := []imws.Frame{
					imws.NewFrame(imws.OpText, false, []byte("fragment1")),
					imws.NewFrame(imws.OpContinuation, false, []byte(",")),
					imws.NewFrame(imws.OpContinuation, true, []byte("fragment2")),
				}
				for _, f := range fs {
			...
			messages: []Message{
				{imws.OpText, []byte("fragment1,fragment2")},
			},
		},
	} {
