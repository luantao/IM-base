...
// is at most imws.MaxControlFramePayloadSize + imws.MaxHeaderSize.
func NewControlWriter(dest io.Writer, state imws.State, op imws.OpCode) *ControlWriter {
	return &ControlWriter{
		w:     NewWriterSize(dest, state, op, imws.MaxControlFramePayloadSize),
		limit: imws.MaxControlFramePayloadSize,
	}
}
...
// imws.MinHeaderSize or imws.MinHeaderSize+4 (depending on state). At most
// (imws.MaxControlFramePayloadSize + x) bytes of buf will be used.
//
// It panics if len(buf) <= imws.MinHeaderSize + x.
func NewControlWriterBuffer(dest io.Writer, state imws.State, op imws.OpCode, buf []byte) *ControlWriter {
	max := imws.MaxControlFramePayloadSize + headerSize(state, imws.MaxControlFramePayloadSize)
	if len(buf) > max {
		buf = buf[:max]
	}
...
func GetWriter(dest io.Writer, state imws.State, op imws.OpCode, n int) *Writer {
	x, m := writers.Get(n)
	if x != nil {
		w := x.(*Writer)
...
// It uses imws.State to decide whether the output frames must be masked.
//
...
func NewWriter(dest io.Writer, state imws.State, op imws.OpCode) *Writer {
	return NewWriterBufferSize(dest, state, op, 0)
}

// NewWriterSize returns a new Writer whose buffer size is at most n + imws.MaxHeaderSize.
...
func NewWriterSize(dest io.Writer, state imws.State, op imws.OpCode, n int) *Writer {
	if n > 0 {
...
// If n <= imws.MinHeaderSize then the default buffer size is used.
//
// Note that Writer will reserve x bytes for header data, where x is in range
// [imws.MinHeaderSize,imws.MaxHeaderSize]. That is, frames flushed by Writer
// will not have payload length equal to n, except the case when Write() is
// called on empty Writer with len(p) > n.
func NewWriterBufferSize(dest io.Writer, state imws.State, op imws.OpCode, n int) *Writer {
	if n <= imws.MinHeaderSize {
...
// [imws.MinHeaderSize,imws.MaxHeaderSize] (depending on state and buf size).
//
// You could use imws.HeaderSize() to calculate number of bytes needed to store
// header data.
//
// It panics if len(buf) is too small to fit header and payload data.
func NewWriterBuffer(dest io.Writer, state imws.State, op imws.OpCode, buf []byte) *Writer {
	w := &Writer{
		dest:  dest,
		state: state,
		op:    op,
...
func (w *Writer) Reset(dest io.Writer, state imws.State, op imws.OpCode) {
	w.dest = dest
	w.state = state
	w.op = op
...
func (w *Writer) ResetOp(op imws.OpCode) {
	w.op = op
	w.n = 0
	w.dirty = false
	w.fseq = 0
...
func (w *Writer) WriteThrough(p []byte) (n int, err error) {
	...
	var frame imws.Frame
	frame.Header = imws.Header{
		OpCode: w.opCode(),
		Fin:    false,
		Length: int64(len(p)),
	}
	...
	if w.state.ClientSide() {
		...
		frame = imws.MaskFrameInPlace(frame)
	} else {
		frame.Payload = p
	}

	w.err = imws.WriteFrame(w.dest, frame)
	if w.err == nil {
		n = len(p)
	}
...
func (w *Writer) opCode() imws.OpCode {
	if w.fseq > 0 {
		return imws.OpContinuation
	}
	return w.op
}
...
func writeFrame(w io.Writer, s imws.State, op imws.OpCode, fin bool, p []byte) error {
	var frame imws.Frame
	if s.ClientSide() {
		// Should copy bytes to prevent corruption of caller data.
		payload := pbytes.GetLen(len(p))
		...
		frame = imws.NewFrame(op, fin, payload)
		frame = imws.MaskFrameInPlace(frame)
	} else {
		frame = imws.NewFrame(op, fin, p)
	}

	return imws.WriteFrame(w, frame)
}

// reserve calculates number of bytes need to be reserved for frame header.
//
// Note that instead of imws.HeaderSize() it does calculation based on the buffer
// size, not the payload size.
func reserve(state imws.State, n int) (offset int) {
	var mask int
	if state.ClientSide() {
		mask = 4
	}
...
func headerSize(s imws.State, n int) int {
	return imws.HeaderSize(imws.Header{
		Length: int64(n),
		Masked: s.ClientSide(),
	})
}
