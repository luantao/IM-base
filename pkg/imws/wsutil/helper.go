...
func ReadMessage(r io.Reader, s imws.State, m []Message) ([]Message, error) {
	rd := Reader{
		Source:    r,
		State:     s,
		CheckUTF8: true,
		OnIntermediate: func(hdr imws.Header, src io.Reader) error {
...
// given io.ReadWriter.
func ReadData(rw io.ReadWriter, s imws.State) ([]byte, imws.OpCode, error) {
	return readData(rw, s, imws.OpText|imws.OpBinary)
}

// ReadClientData reads next data message from rw, considering that caller
// represents server side. It is a shortcut for ReadData(rw, imws.StateServerSide).
//
// Note this may handle and write control frames into the writer part of a
// given io.ReadWriter.
func ReadClientData(rw io.ReadWriter) ([]byte, imws.OpCode, error) {
	return ReadData(rw, imws.StateServerSide)
}
...
func ReadClientText(rw io.ReadWriter) ([]byte, error) {
	p, _, err := readData(rw, imws.StateServerSide, imws.OpText)
	return p, err
}
...
func ReadClientBinary(rw io.ReadWriter) ([]byte, error) {
	p, _, err := readData(rw, imws.StateServerSide, imws.OpBinary)
	return p, err
}

// ReadServerData reads next data message from rw, considering that caller
// represents client side. It is a shortcut for ReadData(rw, imws.StateClientSide).
//
// Note this may handle and write control frames into the writer part of a
// given io.ReadWriter.
func ReadServerData(rw io.ReadWriter) ([]byte, imws.OpCode, error) {
	return ReadData(rw, imws.StateClientSide)
}
...
func ReadServerText(rw io.ReadWriter) ([]byte, error) {
	p, _, err := readData(rw, imws.StateClientSide, imws.OpText)
	return p, err
}
...
func ReadServerBinary(rw io.ReadWriter) ([]byte, error) {
	p, _, err := readData(rw, imws.StateClientSide, imws.OpBinary)
	return p, err
}
...
func HandleClientControlMessage(conn io.Writer, msg Message) error {
	return HandleControlMessage(conn, imws.StateServerSide, msg)
}
...
func HandleServerControlMessage(conn io.Writer, msg Message) error {
	return HandleControlMessage(conn, imws.StateClientSide, msg)
}
...
// were checked by imws.CheckHeader() call.
func HandleControlMessage(conn io.Writer, state imws.State, msg Message) error {
...
func ControlFrameHandler(w io.Writer, state imws.State) FrameHandlerFunc {
	return func(h imws.Header, r io.Reader) error {
		return (ControlHandler{
			DisableSrcCiphering: true,
			Src:                 r,
			Dst:                 w,
...
func readData(rw io.ReadWriter, s imws.State, want imws.OpCode) ([]byte, imws.OpCode, error) {
