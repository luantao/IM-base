...
var ErrUnexpectedCompressionBit = imws.ProtocolError(
...
func UnsetBit(h imws.Header) (_ imws.Header, wasSet bool, err error) {
	var s MessageState
	h, err = s.UnsetBits(h)
...
// modified copy.
// It returns non-nil error if compression bit has unexpected value.
func SetBit(h imws.Header) (_ imws.Header, err error) {
	var s MessageState
...
func IsCompressed(h imws.Header) (bool, error) {
...
func (s *MessageState) UnsetBits(h imws.Header) (imws.Header, error) {
	r1, r2, r3 := imws.RsvBits(h.Rsv)
	switch {
	case h.OpCode.IsData() && h.OpCode != imws.OpContinuation:
		h.Rsv = imws.Rsv(false, r2, r3)
		s.SetCompressed(r1)
		return h, nil
...
func (s *MessageState) SetBits(h imws.Header) (imws.Header, error) {
	r1, r2, r3 := imws.RsvBits(h.Rsv)
	if r1 {
		return h, ErrUnexpectedCompressionBit
	}
	if !h.OpCode.IsData() || h.OpCode == imws.OpContinuation {
		// An endpoint MUST NOT set the "Per-Message Compressed"
		// bit of control frames and non-first fragments of a data
		// message. An endpoint receiving such a frame MUST _Fail
		// the WebSocket Connection_.
