...
data. User decides by himself which data should be copied.
	  ...
	  handshake, err := imws.Upgrade(conn)
	  if err != nil {
		  // handle error
	  }

For customization details see `imws.Upgrader` documentation.
...
That is, `ws` does not force the only one way of how to work with WebSocket:
	  ...
	  resp := imws.NewBinaryFrame([]byte("hello, world!"))
	  if err := imws.WriteFrame(conn, frame); err != nil {
	      // handle err
	  }
...
As you can see, it stream friendly:
	  ...
	  imws.WriteHeader(imws.Header{
		  Fin:    true,
		  Length: N,
		  OpCode: imws.OpBinary,
	  })
