...
//go:generate mockgen -source client.go -destination mock_client.go -self_package github.com/luantao/IM-base/pkg/rocketmq-client-go/internal  --package internal RMQClient
		...
		for id, addr := range data.BrokerAddresses {
			...
			if response.Code == ResSuccess {
				...
				rlog.Warning("send heart beat to broker failed", map[string]interface{}{
					...
					//[ISSUE #1002] fix: add log when send heartbeat failed #1003
...
func (c *rmqClient) UpdateTopicRouteInfo() {
	//[ISSUE #1056] fix: When the consumer and producer have the same topic, the consumer fails to update the topic. #1059
